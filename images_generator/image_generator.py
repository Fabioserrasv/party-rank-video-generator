...
def generate_images():
    with open(JSON_PATH, 'r') as f:
        series = json.load(f)
    sorted_series_names = sorted(series['series'], key=lambda x: (
        series['series'][x]['anime']), reverse=False)
    ...
    sorted_series_names = sorted(series['series'], key=lambda x: (
        series['series'][x]['average']), reverse=True)
    sorted_series = series
    series = {}
    series['series'] = {}
    series['participants'] = sorted_series['participants']
    ...
    for song in series["series"]:
        ...
        text.text((195, 1029), series['series'][song]['anime'], fill=(
            255, 255, 255), font=font482, anchor='lm')  # anime name
        text.text((195, 960), series["series"][song]["song"], fill=(
            255, 244, 79), font=font482, anchor='lm')  # song name
        text.text((195, 891), series["series"][song]["type"], fill=(
            255, 244, 79), font=font362, anchor='lm')  # type
        text.text((51, 80), title, fill=(255, 255, 255),
                  font=font242, anchor='ld')

        text.text((113, 839), str(count), fill=(
            255, 255, 255), font=font60, anchor='mm')
        ...
        for h in range(clear_null):  # ENCONTRA MENOR E MAIOR NOTA
            ...
            if(nota < mn):
                menor = h
                mn = nota
        text.text((1600, 62), "Average: {}".format(
            round(soma/clear_null, 2)), fill=average_note, font=font36, anchor='mm')
        for h in range(clear_null):
            ...
            if h == menor:
                ...
                cor = 2
            text.text(px_nick[h], participante,
                      fill=colors[h], font=font242, anchor='mm')
            text.text(
                px_note[h], nota, fill=colors_note[cor], font=font242, anchor='mm')
            parImg = Image.open(PARTICIPANTS_PATH +
                                "/{}.png".format(participante))
            ...
            image.paste(parImg, px_image[h])

        cover = Image.open(THUMBNAIL_PATH+"/{}".format(series["series"][song]["cover"])).convert(
            'RGB').resize((131, 184), Image.Resampling.LANCZOS)
